...
if ( ! -f "Report/${Design}.timing" || ! -f "Report/${Design}.area" ) then
    echo "${RED}Report files for ${Design} not found.${NO_COLOR}"
    exit 1
endif
...
if ( $status == 0 ) then
    echo "${RED}--> X There is Latch in this design ---${NO_COLOR}"
    @ flag = 1
else
    echo "${GREEN}--> V Latch Checked!${NO_COLOR}"
endif

# Check Width Mismatch
grep -i -q 'mismatch' 'syn.log'
if ( $status == 0 ) then
    echo "${RED}--> X Width Mismatch Error !! ---${NO_COLOR}"
    @ flag = 1
else
    echo "${GREEN}--> V Width Mismatch Checked!${NO_COLOR}"
endif

# Check Error
grep -i -q 'Error' 'syn.log'
if ( $status == 0 ) then
    echo "${RED}--> X There is Error in this design !! ---${NO_COLOR}"
    @ flag = 1
else
    echo "${GREEN}--> V No Error in syn.log!${NO_COLOR}"
endif

# Check Timing
grep -i -q 'violated' "Report/${Design}.timing"
if ( $status == 0 ) then
    echo "${RED}--> X Timing (violated) ---${NO_COLOR}"
    @ flag = 1
else
    echo "${GREEN}--> V Timing (MET) Checked!${NO_COLOR}"
endif

echo "${YELLOW}============================${NO_COLOR}"
if ( $flag == 1 ) then
    echo "${RED}--> X 02_SYN Fail !! Please check out syn.log file.${NO_COLOR}"
else
    echo "${GREEN}--> V 02_SYN Success !!${NO_COLOR}"
endif

echo "${YELLOW}============================${NO_COLOR}"
echo "${YELLOW}Cycle: $Cycle ${NO_COLOR}"
...
echo "${YELLOW}Area: $formatted_area ${NO_COLOR}"

set formatted_gate = `echo $gate_count | awk '{printf "%.0f", $1}'`
echo "${YELLOW}Gate count: $formatted_gate ${NO_COLOR}"

echo "${YELLOW}Dynamic: $Dynamic ${NO_COLOR}"
echo "${YELLOW}Leakage: $Leakage ${NO_COLOR}"